...
class DashBoard extends Component {
  ...
  getAllVotes = () => {
    Axios.get("https://buildingmanage-gh8w.vercel.app/retrieveAllBills").then(
      (response) => {
  ...
  render() {
    ...
    return (
      <div className="Inbox-container">
        ...
        <LeftSide />
        <Navbar title={"dashboard"} desc={"BuildingUpdates"} />
        <RightSide />
            ...
            {this.state.series.length > 0 && (
              <>
                <div className="DashBoardMessage">
                  <span>
                    {"Youhave"} {this.state.unPaidBills} {"billstopay"}
                  </span>
                  <span>
                    {"Total"} : {this.state.unPaidBillsAmount}
                    {"shekel"}{" "}
                  </span>
                </div>
                <Chart
